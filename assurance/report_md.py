...
def render_report(claims: list[dict], meta: dict) -> str:
    ...
    d.lit("Pooled test RMSE ").num(pv.get("rmse_baseline_pooled"), ".4f")
    d.lit(" → ").num(pv.get("rmse_incumbent_pooled"), ".4f")
    d.lit("; effect ").num(pv.get("effect_abs"), ".4f")
    d.lit(" (").num(pv.get("effect_rel_pct"), ".2f").lit("% relative), ")
    d.num(pv.get("confidence_pct"), "d").lit("% CI [")
    d.num(pv.get("ci_lo"), ".4f").lit(", ").num(pv.get("ci_hi"), ".4f").lit("]")
    d.lit(" across ").num(pv.get("n_seeds"), "d").lit(" hidden test seed(s) × ")
    d.num(pv.get("n_examples"), "d").lit(" examples. [").val(primary["claim_id"]).lit("]\n\n")
    d.lit("Statistical test: ").val(primary.get("statistical_test") or "n/a").lit(".\n\n")

    # --- per-seed table ---
    d.lit("## Per-seed reproduction\n\n")
    d.lit("| seed | baseline RMSE | incumbent RMSE | delta |\n")
    d.lit("|------|---------------|----------------|-------|\n")
    for row in pv.get("per_seed") or []:
        d.lit("| s").num(row.get("seed_index"), "d").lit(" | ")
        d.num(row.get("rmse_baseline"), ".4f").lit(" | ")
        d.num(row.get("rmse_incumbent"), ".4f").lit(" | ")
        d.num(row.get("delta"), ".4f").lit(" |\n")
    d.lit("\nSeed consistency (fraction improving): ")
    d.num(pv.get("seed_consistency_pct"), ".2f").lit("%.\n\n")
    d.lit("![per-seed paired RMSE](figures/").val(meta["fig_paired"]).lit(")\n\n")
