...
### Layer 5 — 코딩 executor (Phase 2에서 patcher + ClaudeCoder 구현; Phase 6a에서 실행 OS 격리)
...
**Phase 6a 실행 격리**: 신뢰불가 후보 코드가 실제로 *실행*되는 지점(`evaluate.py`의
`_run_train` — 채점은 호스트 신뢰 경로)을 `Sandbox` 프로토콜로 감싼다. `subprocess`
(현행 동일, 기본) + `ContainerSandbox`(`docker run` — 네트워크 차단·읽기전용 rootfs·
드롭 caps·비루트·리소스 한도·ephemeral PID 네임스페이스, held-out 시드/원장 마스킹).
Docker 데몬/이미지 부재 시 조용히 폴백하지 않고 fail-closed. Linux에서 `--runtime=runsc`
로 gVisor 드롭인, Firecracker·코더 자체 컨테이너화는 후속. (계약 v6 `sandbox` 블록.)
...
- **Phase 5**: 과학적 assurance + 보고. 다중 시드 + 신뢰구간, baseline + ablation,
  ...
  human 승인.
- **Phase 6a**: 실행 격리(Layer 5 심화). `Sandbox` 프로토콜 + Docker `ContainerSandbox`
  로 후보 `train.py` 실행을 OS 격리(네트워크 차단·읽기전용·시드/원장 마스킹·ephemeral
  PID). `subprocess` 기본·`container` 옵트인·fail-closed. gVisor는 `--runtime=runsc`
  드롭인. (다음: 6b 실 문헌 API OpenAlex/S2 + 스냅샷 캐시, 6c 실제 연구 도메인 교체.)
