...
| `sandbox/` | Phase 6a 실행 격리 (`runner.py`): `Sandbox` 프로토콜, `SubprocessSandbox`(현행), `ContainerSandbox`(docker), `build_sandbox`, `preflight`. stdlib·절대경로 로드·런타임 파일IO 없음 | ✅ (§3 폐쇄) |
| `assurance/` | Phase 5 순수 패키지: `stats.py`(paired bootstrap), `claims.py`(claim ledger), `report_md.py`+`figures.py`+`svgfig.py`(결정적 보고/그림), `reviewer.py`(codex 어댑터), `gate.py`(승인 파생), `families.py`(코더 계열) | ✅ (§3-13 폐쇄) |
...
9. **문헌 엔진 폐쇄 (Phase 3)**: `literature/`는 orchestrator를 import하지 않고,
   ...
   `Grounding.move_guidance()`도 같은 폐쇄의 순수 메서드다(범주형 enum + 증거 id만,
   산문·숫자 없음).
10. **search momentum은 파생·미영속 (Phase 4a)**: momentum은 state에 저장되지
    않고 매 세대 ledger의 experiment 레코드에서 재계산된다(`extract_update_vectors`
    → `search_momentum_table`). 입력 폐쇄가 `replay_ledger_fields`와 동일(experiment
...
12. **pairwise는 selection만, admission은 결정적 (Phase 4c)**: admission(gate-split
    ...
    항상 admitted 집합에서만 반환). 심판 입력 폐쇄는 계약·가설 인증서(id만)·bounded
...
13. **assurance 폐쇄 (Phase 5)**: `assurance/`는 stdlib만 import하고 orchestrator·
...
14. **샌드박스 폐쇄·fail-closed·시드 부재** (Phase 6a): `sandbox/`는 신뢰 경로다 —
