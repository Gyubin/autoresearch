...
- **완료**: Phase 1(제약된 keep/reject 루프) + Phase 2(병렬 포트폴리오 + blind
  ...
  human 승인 gate, momentum 코더 계열 분류).
- **다음**: Phase 6(진짜 격리 — 컨테이너/gVisor 샌드박스, 실 문헌 API
  OpenAlex/S2 어댑터, 실제 연구 도메인으로 태스크 교체).
...
| 파일 | 역할 | protected? |
|---|---|---|
| `research_contract.yaml` | 타입드 계약 (schema v5: + assurance/reviewer/human_gate 블록) | ✅ 불변 |
| `orchestrator.py` | 코디네이터 + gate + 코더 + 문헌 시임 + momentum/halving + 승인 게이트 + 다중시드 report + CLI(approve 추가) | ✅ |
...
| `protection/hashes.json` | protected 파일 SHA-256 manifest (20개: +assurance/ 9) | ✅ (git 추적) |
...
`orchestrator.py`에서 찾을 것: `load_contract`(계약 v5 파싱 — 최상위 키
화이트리스트 + halving/refinement/pairwise_gate/assurance/reviewer/human_gate 블록),
...
### Phase 6 — 진짜 격리 + 실 문헌 (Layer 5, 2 심화) ← 다음
- 컨테이너/gVisor/Firecracker 샌드박스로 코더·학습을 OS 수준 격리(현재는 정책적
  가드 훅 + 루트 지문뿐 — TOCTOU·워크스페이스 밖 쓰기·네트워크는 못 막음).
- 실 문헌 API(OpenAlex/S2) 어댑터를 `Retriever` 프로토콜 뒤에 구현.
...
- held-out 시드는 로컬 사용자가 루트에서 읽을 수 있다(정책적 보호). 코더는 가드
  훅으로 못 읽지만 진짜 격리는 컨테이너/가상화 샌드박스 필요 → Phase 6.
...
- Phase 6 미구현: 컨테이너/가상화 샌드박스(진짜 격리), 실 문헌 API 어댑터,
  실제 연구 도메인 태스크 교체. codex 리뷰어는 로컬 `codex` 로그인·계정 한도에
  의존하며(옵트인·advisory), 실패 시 리뷰 부재로 기록될 뿐 보고를 막지 않는다.
- **Phase 5 assurance는 §7의 시드 트러스트 경계를 상속한다**: 학습 서브프로세스는
  OS 샌드박스가 없어 적대적 코더가 `train.py`에서 worktree 밖을 traverse해 숨은
  시드(gate뿐 아니라 이제 test 시드도)를 읽을 수 있고, gitignore된 `experiments/`
  (원장 포함)에 out-of-band로 쓸 수 있다. 그래서 다중 시드 test 재현·paired
  bootstrap·claim ledger·승인 게이트의 보증은 **정직한 후보 + 결정적 파이프라인**
  에 대해 성립하며, headline `primary_effect`는 원장이 아니라 live bootstrap에서만
  나오므로 원장 위조로 못 뒤집지만, 적대적 코더에 대한 진짜 격리는 Phase 6
  샌드박스가 필요하다(적대 리뷰 evaluator-bypass 렌즈 확인). 크래시로 봉인 못 한
  report_attempt는 다중검정 공시에 `crashed_report_attempts`로 보수적으로 집계된다.

## 8. 새 세션 시작 프롬프트 (예시)

> "이 디렉토리(`~/workspace/dev/autoresearch`)의 AutoResearch 시스템에서 Phase 6
> (진짜 격리 샌드박스 + 실 문헌 API)를 이어서 구현하고 싶어. `docs/HANDOFF.md`와
> `docs/BLUEPRINT.md`를 먼저 읽고, 현재 상태를 파악한 뒤 Phase 6 설계안부터
> 제안해줘."
