...
```mermaid
flowchart TD
    C[research_contract.yaml<br/>타입드·불변·protected]:::frozen --> P
    L[문헌 엔진<br/>claim 수준 증거]:::opt -.-> P
    M[search momentum<br/>ledger에서 재계산] -.-> P
    P[Proposer<br/>K개 다양한 가설] --> W[격리된 git worktree<br/>각 1개 파라미터 편집, 병렬 실행]
    W --> SM[smoke rung<br/>successive halving]
    SM --> DEV[dev 스플릿<br/>incumbent 대비 채점]
    DEV --> GATE{blind admission gate<br/>숨겨진 gate 스플릿}
    GATE -->|마진 이상으로 이긴 승자| MERGE[main에 ff-merge]
    GATE -->|나머지 / 일반화 실패| REJ[reject<br/>hyp/* 브랜치 + ledger에 보존]
    MERGE --> INS[insight 증류<br/>momentum 갱신]
    REJ --> INS
    INS -->|다음 generation| P
    MERGE --> REP[report<br/>test 스플릿 · paired-bootstrap CI · claims · human 승인 게이트]:::seal
