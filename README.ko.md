...
*🇺🇸 The English version is the primary README: [README.md](README.md).*
...
이후 단계에서 assurance·보고 계층(Phase 5), 실 OS 실행 격리(Phase 6a), 실 문헌
API(Phase 6b), 실제 연구 도메인(Euclidean-TSP)(Phase 6c)까지 더했다.
...
```mermaid
flowchart TD
    C[research_contract.yaml<br/>타입드·불변·protected]:::base --> P
    L[문헌 엔진<br/>claim 수준 증거]:::optin -.-> P
    M[search momentum<br/>ledger에서 재계산]:::optin -.-> P
    P[Proposer<br/>K개 다양한 가설]:::base --> W[격리된 git worktree<br/>각 1개 파라미터 편집, 병렬 실행]:::base
    W --> SM[smoke rung<br/>successive halving]:::base
    SM --> DEV[dev 스플릿<br/>incumbent 대비 채점]:::base
    DEV --> GATE{blind admission gate<br/>숨겨진 gate 스플릿}:::gate
    GATE -->|마진 이상으로 이긴 승자| MERGE[main에 ff-merge]:::win
    GATE -->|나머지 / 일반화 실패| REJ[reject<br/>hyp/* 브랜치 + ledger에 보존]:::rej
    MERGE --> INS[insight 증류<br/>momentum 갱신]:::base
    REJ --> INS
    INS -->|다음 generation| P
    MERGE --> REP[report<br/>test 스플릿 · paired-bootstrap CI · claims · human 승인 게이트]:::base
...
gate 점수는 ledger 밖으로 절대 나가지 않는다 — proposer·insight·report 어디에도
유입되지 않는다(**blindness** 불변식). `main`에는 gate 통과 실험만 쌓이고, 나머지는
`hyp/*` 브랜치에 provenance로 보존된다. 점선 노드는 옵트인/오프라인 재계산이다.
...
gate blindness·false-repair·literature-closure 불변식을 구조적으로 유지한다.
...
- **SciNav pairwise gate** (LLM 옵트인, `--gate pairwise`): admission은 **항상**
  ...
  diff·**dev** 메트릭만 보고 **gate 점수는 입력조차 받지 못한다**(closure로 blindness
...
### Honest limitations (Phase 6b/6c 작업)
...
- 문헌 엔진은 mock corpus 전용이었다 — 실 API(OpenAlex/S2) 어댑터는 `Retriever`
  시임만 있었고 미구현이었다 (Phase 6b, 이제 완료). 코더 가설의 개입 계열 분류는
  키워드 매칭이라 보수적 (모호하면 분류 포기 → unexplored)
...
- 실 문헌 API(OpenAlex/S2) 어댑터(6b), 실제 연구 도메인 태스크 교체(6c)는 이제
  완료됐다(로드맵 참조). gVisor/Firecracker 백엔드는 Linux에서 `sandbox/runner.py`에
  `--runtime=runsc` 한 줄로 드롭인 가능(후속). 코더 에이전트 자체의 컨테이너화도 후속 하드닝
...
- evidence_steering(patcher)은 정렬 완료: `orchestrator.rank()`/explore 필터가
  ...
  (`tests/test_phase4.py`의 실 corpus end-to-end 드릴로 검증). 다만 공백이 하나 남아 있다 —
