...
uv run python orchestrator.py run --generations 4  # 병렬 포트폴리오 실행 (휴리스틱)
uv run python orchestrator.py status               # 캠페인 상태
uv run python orchestrator.py report               # test 스플릿 최종 보고 (1회성)
uv run python orchestrator.py verify-protection
```

LLM 가설 생성기 + 코딩 워커(Claude Agent SDK — 로컬 Claude Code 로그인 재사용,
별도 API 키 불필요). 코더 가설은 `--proposer claude`일 때만 생성된다:

```bash
uv run python orchestrator.py run --generations 4 --proposer claude
```
...
- 문헌 근거 엔진(claim-level evidence)·claim-evidence ledger 기반 보고서·
  cross-model 적대적 리뷰어는 아직 없음 (블루프린트 Phase 3~5)

## 파일 구조

```
research_contract.yaml    # Layer 1 타입드 계약 (v2: portfolio/gate) — 불변, protected
orchestrator.py           # 포트폴리오 코디네이터 + gate + 코더 (protected)
...
experiments/              # 런타임: state.json, ledger.jsonl, rounds/, generations/gate,
                          #   report/ (gitignored)
...
experiments/를 비우므로 이전 캠페인 기록이 필요하면 먼저 백업할 것. Phase 2는
계약·평가기 schema가 v2라, v1 상태(state.json)에서 이어 돌릴 수 없고
`init --force`로 새 캠페인을 시작해야 한다.

## 남은 로드맵 (블루프린트 기준)

Phase 1(제약된 keep/reject) + Phase 2(포트폴리오·blind gate·LLM 코더) 완료. 다음:

3. 문헌 근거 엔진 (claim-level evidence, PaperQA2류) — 코드가 문헌을 직접 실행
   못 하게 격리
4. Gome 스타일 branch-local directed update + SciNav 스타일 pairwise 평가
   (스칼라 지표가 불충분할 때)
5. claim-evidence ledger 기반 보고서 생성 + cross-model 적대적 리뷰어 (실행기와
   다른 모델 계열) + 컨테이너/가상화 샌드박스로 실제 격리
