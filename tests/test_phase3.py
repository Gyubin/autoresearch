...
def ground(engine: EvidenceEngine, **overrides):
    kwargs = dict(objective="Minimize held-out RMSE of the model trained by "
                            "src/train.py.",
                  hyperparams={"lr": 0.005, "momentum": 0.0, "l2": 0.0,
...
def test_corpus_integrity(tmp: Path) -> None:
    ...
    check("corpus: 13 papers / 15 claims",
          len(corpus.papers) == 13 and len(corpus.claims) == 15,
          f"{len(corpus.papers)}/{len(corpus.claims)}")
    rel = [r for r in corpus.claims["cl_0402"].get("relations") or []
           if r.get("type") == "contradicts" and r.get("to_claim") == "cl_0401"]
    check("corpus: L2 contradiction pair annotated", bool(rel))
    check("corpus: injection fixture flagged",
          corpus.claims["cl_0901"].get("fixture") == "prompt_injection"
          and INJECTION_MARKER in corpus.claims["cl_0901"]["claim"])
    momentum_paper = corpus.papers["mock:2021.0013"]
    searchable = (momentum_paper["title"] + momentum_paper["abstract"]
                  + " ".join(s.get("text", "") for s in momentum_paper["sections"])
                  + " ".join(momentum_paper["concepts"])
                  + corpus.claims["cl_1201"]["claim"]).lower()
    check("corpus: traversal-only paper avoids lexical tokens",
          "momentum" not in searchable and "beta" not in searchable)
...
def test_deterministic_reproducibility() -> None:
    ...
    hyp_a = mk_hyp("h_x", "l2", 0.0, 0.01)
    hyp_b = mk_hyp("h_x", "l2", 0.0, 0.01)
...
def test_hypothesis_ids_only() -> None:
    ...
    hyps = [mk_hyp("h_r0001_feature_scaling", "feature_scaling", False, True),
            mk_hyp("h_r0002_coder", None,
                   statement="add x0*x1 interaction feature",
                   executor="coder",
                   brief="extend FEATURE_SPEC with product interaction terms")]
    ...
    check("ids-only: feature_scaling hypothesis cites the supporting claim",
          "ev_cl-0101" in hyps[0].supporting_evidence_ids)
    check("ids-only: coder hypothesis cites the interaction claim",
          "ev_cl-0501" in hyps[1].supporting_evidence_ids)
...
def test_gate_canary() -> None:
    ...
    engine = make_engine()
    grounding = ground(engine)
    hyps = [mk_hyp("h_r0001_l2", "l2", 0.0, 0.01)]
    engine.attach(hyps, grounding)
    surfaces = {
    ...
    ctx = orch.ProposalContext(
        ...
        current_hyperparams={"lr": 0.005, "l2": 0.0},
...
def test_citation_whitelist() -> None:
    ...
    ctx = orch.ProposalContext(
        contract=contract, round_index=1,
        current_hyperparams={"lr": 0.005, "momentum": 0.0, "l2": 0.0,
                             "batch_size": 32, "epochs": 30,
                             "feature_scaling": False},
        best_primary=0.38, tested={}, last_accepted=None, insights=[],
        evidence=grounding.proposer_view())
    raw = {"statement": "enable feature scaling to equalize curvature",
           "mechanism": "conditioning", "executor": "patcher",
           "param": "feature_scaling", "new_value": True,
           "implementation_brief": None, "predicted_effect": "lower rmse",
           "falsifier": "no change",
    ...
    hyp = mk_hyp("h_r0009_l2", "l2", 0.0, 0.01, cited=["ev_BOGUS"])
    engine.attach([hyp], grounding)
    report = grounding.novelty["per_hypothesis"]["h_r0009_l2"]
...
def test_anti_laundering() -> None:
    engine = make_engine()
    grounding = ground(engine)
    trap = grounding.record("ev_cl-0801")  # tree-ensemble scaling claim
    check("laundering: trap claim retrieved but adjacent",
          trap is not None and trap["stance"] == "adjacent")

    hyp = mk_hyp("h_r0001_feature_scaling", "feature_scaling", False, True,
                 cited=["ev_cl-0801"])
    engine.attach([hyp], grounding)
    report = grounding.novelty["per_hypothesis"]["h_r0001_feature_scaling"]
    check("laundering: forged citation stripped with stance reason",
          "ev_cl-0801" not in hyp.supporting_evidence_ids
          and any(e["evidence_id"] == "ev_cl-0801"
                  and e["reason"].startswith("stance_")
                  for e in report["laundering_filtered"]))

    # A cross-model-class pair is not a genuine contradiction: the trap
    # claim must never be paired against the on-model-class scaling result.
    check("laundering: no cross-model-class contradiction surfaced",
          not any("ev_cl-0801" in c["evidence_ids"]
    ...
    coder_hyp = mk_hyp(
        ...
        statement="Increase the batch size aggressively",
        brief="use an effective adaptive step per size bucket in training")
    fam, move = engine._coder_family(
        *engine._hypothesis_family(coder_hyp)[2:])
    check("coder-family: contiguous phrase matching picks batch_size",
          fam == "batch_size", f"got {fam}/{move}")
    ...
    sideways = llm_grounding.record("ev_cl-0401")
...
def test_novelty_categorical() -> None:
    ...
    grounding = ground(engine)
    hyps = [
        mk_hyp("h_fs", "feature_scaling", False, True),
        mk_hyp("h_l2", "l2", 0.0, 0.01),
        mk_hyp("h_dropout", None, executor="coder",
               statement="add dropout regularization to the trainer",
               brief="implement random unit dropout inside the training loop"),
    ]
    engine.attach(hyps, grounding)
    per = grounding.novelty["per_hypothesis"]
    check("novelty: feature_scaling -> replication",
          per["h_fs"]["novelty_category"] == "replication",
          per["h_fs"]["novelty_category"])
    check("novelty: dropout coder brief -> unexplored",
          per["h_dropout"]["novelty_category"] == "unexplored",
          per["h_dropout"]["novelty_category"])
    ...
    # An l2-up hypothesis meets the contradicting weight-decay claim.
    l2_relations = {c["relation"] for c in per["h_l2"]["nearest_prior_claims"]}
    check("novelty: l2 nearest priors include a contradiction",
          per["h_l2"]["novelty_category"] in ("contradiction_test",
                                              "replication")
          and bool(l2_relations))
...
def test_citation_traversal() -> None:
    # "momentum" alone: hits the heavy-ball paper's PARENT lexically, while
    # the negative-result paper itself avoids the token by construction.
    query = [{"topic": "momentum", "query": "momentum"}]
    ...
    check("traversal: hop-1 surfaces the heavy-ball negative result",
...
def test_injection_confinement(tmp: Path) -> None:
    engine = make_engine()
    grounding = ground(engine)
    moves = [("feature_scaling", False, True), ("lr", 0.005, 0.008),
             ("momentum", 0.0, 0.9), ("l2", 0.0, 0.01),
             ("batch_size", 32, 16), ("epochs", 30, 45)]
    hyps = [mk_hyp(f"h_{p}", p, f, t) for p, f, t in moves]
    hyps.append(mk_hyp("h_coder", None, executor="coder",
                       statement="add x0*x1 product interaction terms",
                       brief="extend FEATURE_SPEC with pairwise products"))
    engine.attach(hyps, grounding)
    check("injection: fixture never in supporting_evidence_ids",
          all("ev_cl-0901" not in h.supporting_evidence_ids for h in hyps))
...
def test_empty_and_disabled(tmp: Path) -> None:
    ...
    check("relative corpus_path resolves against corpus_root",
          len(engine_rooted.corpus.claims) == 15)
...
def test_contract_literature(tmp: Path) -> None:
    ...
    expect_reject("corpus path traversal",
                  text.replace(
                      ...
                      'heldout_config.json"'))
    expect_reject("unimplemented retriever",
                  text.replace("retriever: lexical", "retriever: openalex"))
